...
class PermissionService {
    ...
    async checkUserPermission(userId, permissionKey) {
        ...
        if (userPermissions.includes(permissionKey)) {
            ...
            for (const role of rolesWithPermission) {
                if (role.scope && role.scope.level === "global") {
                    ...
                    role.scope &&
                    this.compareScopeLevel(role.scope.level, scope.level) > 0) {
                    scope = role.scope;
                }
            }
            return {
        ...
        if (userPermissions.includes(`${module}:*`)) {
            ...
            return {
                hasPermission: true,
                scope: rolesWithModuleAccess.length > 0 && rolesWithModuleAccess[0]
                    ? rolesWithModuleAccess[0].scope
                    : undefined,
    ...
    async getPermissionStats() {
        ...
        permissions.forEach(permission => {
            if (permission.module) {
                ...
                stats.permissionsByModule[permission.module] =
            ...
            if (permission.group) {
                ...
                stats.permissionsByGroup[permission.group] =
                    (stats.permissionsByGroup[permission.group] || 0) + 1;
            }
        });
        return stats;
    }
    ...
    async updatePermission(id, updateData) {
        const permission = await models_1.Permission.findByIdAndUpdate(id, updateData, {
            new: true,
            runValidators: true,
        });
        if (!permission) {
