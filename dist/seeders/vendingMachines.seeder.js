...
exports.seedVendingMachines = void 0;
const VendingMachine_model_1 = require("../models/VendingMachine.model");
const logger_util_1 = require("../utils/logger.util");
const seedVendingMachines = async (createdBy = null, productMap) => {
    try {
        logger_util_1.logger.info("🌱 Seeding vending machines...");
        const existingCount = await VendingMachine_model_1.VendingMachine.countDocuments();
        if (existingCount > 0) {
            logger_util_1.logger.info(`✅ Vending machines already seeded (${existingCount} machines found)`);
            const existingMachines = await VendingMachine_model_1.VendingMachine.find();
            const machineMap = {};
        ...
        const machines = [
            {
                ...
                productSlots: [
                    ...
                    {
                        slotNumber: "A4",
                        product: productMap["mountain_dew"],
                        quantity: 5,
                        maxCapacity: 8,
                        price: 30,
                    },
                    {
                        slotNumber: "B1",
                        product: productMap["bisleri_water"],
                        quantity: 12,
                        maxCapacity: 15,
                        price: 20,
                    },
                    ...
                    {
                        slotNumber: "B4",
                        product: productMap["nescafe_cold_coffee"],
                        quantity: 6,
                        maxCapacity: 8,
                        price: 35,
                    },
                    {
                        slotNumber: "C1",
                        product: productMap["lays_classic"],
                        quantity: 8,
                        maxCapacity: 12,
                        price: 20,
                    },
                    ...
                    {
                        slotNumber: "C3",
                        product: productMap["oreo_cookies"],
                        quantity: 7,
                        maxCapacity: 10,
                        price: 30,
                    },
                    {
                        slotNumber: "C4",
                        product: productMap["dairy_milk_chocolate"],
                        quantity: 5,
                        maxCapacity: 8,
                        price: 40,
                    },
            ...
            },
            {
                ...
                productSlots: [
                    ...
                    {
                        slotNumber: "B2",
                        product: productMap["granola_bar"],
                        quantity: 7,
                        maxCapacity: 10,
                        price: 40,
                    },
                    {
                        slotNumber: "B3",
                        product: productMap["parle-g_biscuits"],
                        quantity: 10,
                        maxCapacity: 15,
                        price: 10,
                    },
            ...
            },
            {
                ...
                productSlots: [
                    ...
                    {
                        slotNumber: "B3",
                        product: productMap["bingo_mad_angles"],
                        quantity: 10,
                        maxCapacity: 15,
                        price: 25,
                    },
        ...
        const createdMachines = await VendingMachine_model_1.VendingMachine.insertMany(machinesWithCreatedBy);
        logger_util_1.logger.info(`✅ Successfully seeded ${createdMachines.length} vending machines`);
        const machineMap = {};
        createdMachines.forEach(machine => {
            machineMap[machine.machineId] = machine._id;
        });
        const machineNames = createdMachines.map(m => `${m.name} (${m.machineId})`);
        logger_util_1.logger.info(`🏪 Created vending machines: ${machineNames.join(", ")}`);
        return machineMap;
    }
    catch (error) {
        logger_util_1.logger.error("❌ Error seeding vending machines:", error);
        throw error;
    }
};
exports.seedVendingMachines = seedVendingMachines;
if (require.main === module) {
    Promise.resolve().then(() => __importStar(require("../config/database"))).then(({ connectDB }) => {
        Promise.resolve().then(() => __importStar(require("./products.seeder"))).then(({ seedProducts }) => {
            connectDB().then(() => {
                seedProducts().then(productMap => {
                    (0, exports.seedVendingMachines)(null, productMap)
                        .then(() => {
                        process.exit(0);
                    })
                        .catch(error => {
                        logger_util_1.logger.error("Failed to seed vending machines:", error);
                        process.exit(1);
                    });
                });
            });
