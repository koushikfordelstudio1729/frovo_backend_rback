...
const checkScopedAccess = async (user: any, entityId?: string): Promise<boolean> => {
  ...
  for (const role of user.roles) {
    ...
    if (role.scope.entities && role.scope.entities.some((id: Types.ObjectId) => id.toString() === entityId)) {
      return true;
    }
  }
...
export const requireRole = (roleKey: string) => {
  return asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const hasRole = req.user.roles.some((role: any) => 
      role.key === roleKey || role.systemRole === roleKey
    );
...
export const requireSystemRole = (systemRole: SystemRole) => {
  return asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const hasSystemRole = req.user.roles.some((role: any) => 
