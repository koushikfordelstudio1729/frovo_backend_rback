...
export class MachineService {
    ...
    try {
      ...
      if (machineData.machineId) {
        ...
        if (!machineIdPattern.test(machineData.machineId)) {
          throw new Error(`Invalid machineId format. Expected format: VMYYYYNNN (e.g., VM2026001)`);
        }
        const currentYear = new Date().getFullYear();
        const providedYear = parseInt(machineData.machineId.substring(2, 6));

        if (providedYear < currentYear) {
          throw new Error(
            `Cannot create machine with past year ${providedYear}. Current year is ${currentYear}`
          );
        }

        if (providedYear > currentYear) {
          throw new Error(
            `Cannot create machine with future year ${providedYear}. Current year is ${currentYear}`
          );
        }

        const existingMachine = await Machine.findOne({ machineId: machineData.machineId });
        if (existingMachine) {
          throw new Error(`Machine with ID ${machineData.machineId} already exists`);
        }
      }
  ...
  static async checkMachineExists(machineId: string): Promise<boolean> {
    try {
      const machine = await Machine.findOne({ machineId }).select("machineId").lean();
      return !!machine;
    } catch (error) {
  ...
  static async getAuditTrailsByMachineId(
    ...
    const { limit = 4, skip = 0, startDate, endDate, action, entityType } = options ?? {};

    const elemMatch: Record<string, any> = {};
    if (startDate) elemMatch.timestamp = { ...elemMatch.timestamp, $gte: startDate };
    if (endDate) elemMatch.timestamp = { ...elemMatch.timestamp, $lte: endDate };
    if (action) elemMatch.action = action;
    if (entityType) elemMatch.entityType = entityType;

    const hasFilter = Object.keys(elemMatch).length > 0;

    const [result] = await Machine.aggregate([
      { $match: { _id: new mongoose.Types.ObjectId(machineId) } },
        ...
        ? [
            {
              $match: Object.fromEntries(
                Object.entries(elemMatch).map(([k, v]) => [`auditLogs.${k}`, v])
              ),
            },
          ]
        : []),
...

      {
        $facet: {
          ...
          machineInfo: [{ $limit: 1 }, { $project: { _id: 1, serialNumber: 1, modelNumber: 1 } }],
        },
      },
    ]);

    if (!result.machineInfo?.length) throw new Error("Machine not found");

    const total = result.meta[0]?.total ?? 0;
    const { _id, serialNumber, modelNumber } = result.machineInfo[0];

    return { machineId: _id, serialNumber, modelNumber, total, logs: result.logs };
  }
