...
class PaymentService {
  ...
  async processPaymentWebhook(webhookData: PaymentWebhookData) {
    const { paymentId, gatewayTransactionId, status, errorCode, errorMessage, rawResponse } =
      webhookData;
  ...
  async processRefund(refundData: RefundData) {
    ...
    const refundPayment = new Payment({
      ...
      transactionType:
        refundAmount === payment.amount ? TransactionType.REFUND : TransactionType.PARTIAL_REFUND,
    ...
    if (refundResult.success) {
      await (refundPayment as any)["markAsSuccessful"]({
        gatewayTransactionId: refundResult.refundId,
      });
      await (payment as any)["processRefund"](refundAmount, refundResult.refundId);
    } else {
      await (refundPayment as any)["markAsFailed"](
        (refundResult as any).errorCode,
        (refundResult as any).errorMessage
      );
    }
