...
class WarehouseService {
  ...
  private async generatePendingVsRefillData(
    _warehouseId?: string,
    _filters?: any
  ): Promise<{
    days: string[];
  ...
  async updateGRNStatus(
    ...
    try {
      ...
      const updatedGRN = await GRNnumber.findByIdAndUpdate(grnId, updateData, {
        new: true,
        runValidators: true,
      })
        .populate("vendor")
    ...
    try {
      ...
      lineItems.forEach(item => {
        ...
        if (item.received_quantity !== item.accepted_quantity + item.rejected_quantity) {
          throw new Error(
            `Received quantity must equal accepted + rejected for line ${item.line_no}`
          );
        }
      });
  ...
  async updatePurchaseOrderStatus(
    ...
    const updatedPO = await RaisePurchaseOrder.findByIdAndUpdate(id, updateData, { new: true })
  ...
  async updateDispatchStatus(dispatchId: string, status: string): Promise<IDispatchOrder> {
    ...
    const dispatch = await DispatchOrder.findByIdAndUpdate(dispatchId, updateData, {
      new: true,
  ...
  async getQCTemplates(sku?: string): Promise<IQCTemplate[]> {
    ...
    return await QCTemplate.find(query).populate("createdBy", "name email").sort({ title: 1 });
  }

  async updateQCTemplate(
    templateId: string,
    updateData: Partial<QCTemplateData>
  ): Promise<IQCTemplate | null> {
    if (!Types.ObjectId.isValid(templateId)) return null;

    return await QCTemplate.findByIdAndUpdate(templateId, updateData, { new: true }).populate(
      "createdBy",
      "name email"
    );
  }
  ...
  async createReturnOrder(data: ReturnOrderData, createdBy: Types.ObjectId): Promise<IReturnOrder> {
    ...
    if (inventory.quantity < quantity) {
      throw new Error(
        `Insufficient quantity in batch. Available: ${inventory.quantity}, Requested: ${quantity}`
      );
    }
  ...
  private determineReturnType(
    reason: string
  ): "damaged" | "expired" | "wrong_item" | "overstock" | "other" {
    const lowerReason = reason.toLowerCase();

    if (
      lowerReason.includes("damage") ||
    ...
    if (
      lowerReason.includes("overstock") ||
  ...
  async getInventoryDashboard(
    ...
    const query: any = { warehouse: new Types.ObjectId(warehouseId) };
  ...
  async updateInventoryItem(
    ...
    const finalExpiryDate =
      updates.expiryDate !== undefined ? updates.expiryDate : inventory.expiryDate;
    const finalMinStock =
      updates.minStockLevel !== undefined ? updates.minStockLevel : inventory.minStockLevel;
    const finalMaxStock =
      updates.maxStockLevel !== undefined ? updates.maxStockLevel : inventory.maxStockLevel;
    ...
    const updated = await Inventory.findByIdAndUpdate(inventoryId, updates, {
      new: true,
      runValidators: true,
    })
      .populate("warehouse", "name code")
  ...
  async updateExpenseStatus(
    ...
    const expense = await Expense.findByIdAndUpdate(expenseId, updateData, { new: true }).populate(
      "vendor warehouseId createdBy approvedBy"
    );
  ...
  private async generateInventorySummaryReport(filters: any): Promise<InventorySummaryReport> {
    ...
    const inventoryData = await Inventory.find(inventoryQuery).populate("warehouse", "name code");
    ...
    const nearExpirySKUs = inventoryData.filter(
      item => item.expiryDate && item.expiryDate <= next30Days && item.expiryDate >= today
    ).length;
      ...
      summary: {
        ...
        averageTurnover:
          turnoverData.length > 0
            ? turnoverData.reduce((acc: number, item: any) => acc + item.turnoverRate, 0) /
              turnoverData.length
            : 0,
  ...
  private async generateQCSummaryReport(filters: any): Promise<any> {
    ...
    const passRate =
      totalReceivings > 0
        ? ((qcData.find((item: any) => item._id === "approved")?.count || 0) / totalReceivings) *
          100
        : 0;
  ...
  private convertInventorySummaryToCSV(data: any): string {
    ...
    data.inventoryDetails.forEach((item: any) => {
      const threshold = this.getStockThreshold(
        item.quantity,
        item.minStockLevel,
        item.maxStockLevel
      );
  ...
  async deleteWarehouse(warehouseId: string) {
    ...
    if (inventoryCount > 0) {
      throw new Error(
        "Cannot delete warehouse with active inventory. Please archive or transfer inventory first."
      );
    }
