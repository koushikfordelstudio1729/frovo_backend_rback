...
import { sendSuccess, sendCreated, sendError } from '../utils/responseHandlers'

// Screen 1: Dashboard
export const getDashboard = asyncHandler(async (req: Request, res: Response) => {
  try {
    const dashboard = await warehouseService.getDashboard(
      (req.query as any).warehouseId as string,
      req.query
    );
...
export const getReceivings = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const { warehouseId, ...filters } = req.query as any;
...
export const getReceivingById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { id } = req.params;
  if (!id) return sendError(res, 'Receiving ID is required', 400);

  try {
    const result = await warehouseService.getReceivingById(id);
    if (!result) return sendError(res, 'Receiving record not found', 404);
...
export const updateDispatchStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!id) return sendError(res, 'Dispatch ID is required', 400);
  if (!status) return sendError(res, 'Dispatch status is required', 400);
...
export const getQCTemplates = asyncHandler(async (req: Request, res: Response) => {
  try {
    const templates = await warehouseService.getQCTemplates((req.query as any).category as string);
...
export const getReturnQueue = asyncHandler(async (req: Request, res: Response) => {
  try {
    const returnQueue = await warehouseService.getReturnQueue((req.query as any).warehouseId as string);
...
export const approveReturn = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!id) return sendError(res, 'Return ID is required', 400);
...
export const updateInventory = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!id) return sendError(res, 'Inventory ID is required', 400);
...
export const getLowStockAlerts = asyncHandler(async (req: Request, res: Response) => {
  try {
    const alerts = await warehouseService.getLowStockAlerts((req.query as any).warehouseId as string);
...
export const getStockAgeingReport = asyncHandler(async (req: Request, res: Response) => {
  try {
    const report = await warehouseService.getStockAgeingReport((req.query as any).warehouseId as string);
...
export const updateExpenseStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!id) return sendError(res, 'Expense ID is required', 400);
  if (!status) return sendError(res, 'Expense status is required', 400);
